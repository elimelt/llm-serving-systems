...
class Engine:
    ...
    def run(self, input_ids, *, prefill: bool = True) -> torch.Tensor:
        ...
        input_ids : List[torch.Tensor]
            - prefill=True   ->  each tensor is (S_i,)  (full, *variable-length* prompt)
            - prefill=False  ->  each tensor is (1,)    (last token from previous step)
        prefill : bool
        ...
        if prefill or not self.kv_cache:
            self.kv_cache = [ [{} for _ in range(self.layers)] for _ in range(batch_size) ]
        ...
        for layer in range(self.layers):
            ...
            for b in range(batch_size):
                ...
                x_norm  = (x / rms).to(torch.float16) * self.weights["layernormAttn_weight"][layer]

                # projections
                k = x_norm @ self.weights["self_attn_k_proj_weight"][layer].T # (T, d_model)
                ...
                if prefill:
                    ...
                    causal = torch.tril(torch.ones(Lq, Ttot, dtype=torch.bool, device=scores.device))
                    scores = scores.masked_fill(~causal.unsqueeze(0), float("-inf"))

                attn = torch.softmax(scores, dim=-1)
                v_t  = v_rep.permute(1, 0, 2)                               # (H_q, T_tot, d_h)
                ctx  = attn @ v_t                                           # (H_q, L_q, d_h)
                ctx  = ctx.permute(1, 0, 2).reshape(last_q.shape[0], -1)    # (L_q, H_q*d_h)
                ...
                if prefill:
                    ...
                    hid_states[b] = torch.cat([hid_states[b][:-1], hid_states[b][-1:] + y], dim=0)

                # ----------------------- feed-forward -----------------------
                x = hid_states[b]
                rms = torch.sqrt(x.pow(2).mean(-1, keepdim=True) + 1e-5)
                x_norm = (x / rms).to(torch.float16) * self.weights["layernormFFN_weight"][layer]
        ...
        for b in range(batch_size):
            x = hid_states[b]
            rms = torch.sqrt(x.pow(2).mean(-1, keepdim=True) + 1e-5)
            x_norm = (x / rms).to(torch.float16) * self.weights["model_layernorm_weight"]
            logits = x_norm @ self.weights["lm_head_weight"].T
            next_tokens.append(logits[-1].argmax().item())

        return torch.tensor(next_tokens, dtype=torch.int32).to(device=input_ids[0].device)
